...
### Gasless, batched pay runs (smart payroll wallet, opt-in)

With `PAYRUN_WALLET_KIND=smart` the payroll wallet is a CDP smart account driven through AgentKit's `CdpSmartWalletProvider`. The agent still decides one invoice at a time; each approved transfer is queued, the balance it sees is net of the queue, and when the run ends every payout settles in one user operation (one signature, one transaction, gas sponsored on Base Sepolia, so the wallet holds no ETH). The wallet rules compile to `sendUserOperation` rules on the owner account that signs, so every call in a batch must be a USDC `transfer` to a contractor-book wallet within its cap; a refused batch is recorded as refused on every invoice in it. `npm run cli -- smart address|apply|attack|payroll`. Run `smart attack` after `smart apply` and before relying on it: it asks the smart wallet to pay an outside address and must be refused. Offline walkthrough: `npm run demo:batch`.
